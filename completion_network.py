...
class CompletionNet(nn.Module):
    ...
    ENC_CHANNELS = [2, 4, 8, 16, 32, 64, 128, 256, 256]
    DEC_CHANNELS = [2, 4, 8, 16, 32, 64, 128, 256, 256]
    ...
    def __init__(self, resolution):
        ...
        self.enc_block_s64_128 = nn.Sequential(
            ME.MinkowskiConvolution(
        ...
        self.enc_block_s128_256 = nn.Sequential(
            ME.MinkowskiConvolution(
                enc_ch[7], enc_ch[8], kernel_size=2, stride=2, dimension=3
            ),
        ...
        self.dec_s128_cls = ME.MinkowskiConvolution(
            dec_ch[7], 1, kernel_size=1, bias=True, dimension=3
        )
        ...
        self.dec_s64_cls = ME.MinkowskiConvolution(
            dec_ch[6], 1, kernel_size=1, bias=True, dimension=3
        )
        ...
        self.dec_features = ME.MinkowskiConvolution(
            dec_ch[0], 1, kernel_size=1, dimension=3
        )
    ...
    def forward(self, partial_in, target_key):
        ...
        enc_s128 = self.enc_block_s64_128(enc_s64)
        enc_s256 = self.enc_block_s128_256(enc_s128)


        ##################################################
        # Decoder 256 -> 128
        ##################################################
        dec_s128 = self.dec_block_s256s128(enc_s256)

        # Add encoder features
        dec_s128 = dec_s128 + enc_s128
        dec_s128_cls = self.dec_s128_cls(dec_s128)
        keep_s128 = (dec_s128_cls.F > 0).squeeze()

        target = self.get_target(dec_s128, target_key)
        targets.append(target)
        out_cls.append(dec_s128_cls)

        if self.training:
            keep_s128 += target

         # Remove voxels s128
        dec_s128 = self.pruning(dec_s128, keep_s128)
        ##################################################
        # Decoder 128 -> 64
        ##################################################
        dec_s64 = self.dec_block_s128s64(dec_s128)
        ...
        target = self.get_target(dec_s64, target_key)
        targets.append(target)
        ...
        # Decoder 64 -> 32
        ##################################################
        dec_s32 = self.dec_block_s64s32(dec_s64)
        ...
        out_features = self.dec_features(dec_s1)
        return out_cls, targets, dec_s1, out_features
